...
export function BarcodeScanner({ onScan, onClose }: BarcodeScannerProps) {
  ...
  return (
    <div className="fixed inset-0 bg-black z-[9999] flex items-center justify-center p-0 md:p-6 select-none">
      <motion.div 
        initial={{ opacity: 0, scale: 0.95 }}
        animate={{ opacity: 1, scale: 1 }}
        exit={{ opacity: 0, scale: 0.95 }}
        className="w-full h-full md:max-w-3xl md:h-[80vh] bg-slate-950 md:rounded-[2.5rem] shadow-neon-indigo border border-white/5 overflow-hidden flex flex-col ring-1 ring-white/10 relative"
      >
        <div className="p-5 border-b border-white/5 flex items-center justify-between bg-black select-none z-20">
        ...
        <div className="p-0 bg-black flex-1 relative flex items-center justify-center overflow-hidden touch-none group">
          {errorMsg ? (
            <div className="flex flex-col items-center justify-center text-rose-500 p-8 text-center bg-rose-50/10 w-full h-full">
            ...
            <>
              <video
                ref={videoRef}
                className="w-full h-full object-cover pointer-events-none brightness-[1.1] contrast-[1.1] absolute inset-0"
                muted
                playsInline
              />
